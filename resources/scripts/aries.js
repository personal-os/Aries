...
function goThere() {
  ...
  $("webview.active").on("did-start-loading", function () {
    ...
    console.log(
      // "URL when loading started: " + $("webview.active")[0].src
    );
  });

  $("webview.active").on("did-stop-loading", function () {
    NProgress.done();

    console.log(
      // "URL when loading stopped: " + $("webview.active")[0].src
    );
